package utils.algorithms.cmaes;
 ...
 * evolution strategy, it will usually take about 30&#215; N  function
 ...
 * { #updateDistribution(double[])} or { #updateDistribution(double[][], double[])}. 
 ...
 *   
 ...
 The implementation follows very closely. It supports small and large 
...
is implemented in class { cmaes.examples.CMAExample2}.

 * Practical hint: In order to solve an optimization problem in reasonable time it needs to be 
 ...
 * see http://www.lri.fr/~hansen/cmaes_inmatlab.html#practical here for a few more details.
 * 

References

[1] Hansen, N. and A. Ostermeier (2001). Completely
Derandomized Self-Adaptation in Evolution Strategies. Evolutionary
Computation, 9(2), pp. 159-195. 

[2] Hansen, N., S.D. M&uuml;ller and
P. Koumoutsakos (2003). Reducing the Time Complexity of the
Derandomized Evolution Strategy with Covariance Matrix Adaptation
(CMA-ES). Evolutionary Computation, 11(1), pp. 1-18.

[3] Hansen and Kern (2004). Evaluating the CMA Evolution
...
With Increasing Population Size. In Proceedings of the IEEE
...
In Rudolph et al. (eds.) Parallel Problem Solving from Nature, PPSN X,
Proceedings, pp. 296-305, Springer.
...
public class CMAEvolutionStrategy implements java.io.Serializable {
    ...
    int N;
    Random rand = RandUtils.getRNG(); 
    long seed = RandUtils.getSeed();
    ...
    //XXX (gio) added numerical matrix reparation
    boolean repairMatrix = false;
    ...
    /** retrieves options and strategy parameters from properties input, see file CMAEvolutionStrategy.properties 
    ...
    */
    ...
    //  Fortran subroutine in EISPACK.
       ...
       for (int l = 0; l < n; l++) {
          ...
          // XXX (gio) added (m < n) check to prevent ArrayIndexOutOfBoundsException
          if (m > l && m < n) {
             int iter = 0;
             do {
    ...
    public double[][] samplePopulation() {
        ...
        else
        {
        	...
        	if (repairMatrix)
        	{
        		int maxIter = 5;		// maximum number of matrix correction iterations
    			double delta = 1.5;		// matrix correction factors
    			boolean stop = false;
    			double k = 1;
    			for (int iter = 0; iter < maxIter && !stop; iter++)
    			{
    				...
    				{
    					//warning("an eigenvalue has become negative. applying matrix correction (iteration #" + iter +")");
    					minDiag = Math.abs(minDiag);
    					for (i = 0; i < N; i++)
    					{
    						for (j = 0; j < N; j++)
    						{
     ...
     * { #updateDistribution(double[] functionValues)}. functionValues is used to establish an 
     * ordering of the elements in population. The first nInjected elements do not need to originate 
     * from #samplePopulation() or can have been modified (to be tested). 
     * 
     * @param population  double[lambda][N], lambda solutions
     * @param functionValues  double[lambda], respective objective values of population
...
//        }
     ...
     * 
     * @param fitness function value computed for the solution { #getMeanX()}
...
/** search space dimensions must be set before the optimization is started. */
     ...
     * or { #setInitialX(double[])} function call. 
    ...
    public void setSeed(long seed) { 
         ...
         * 5.5061568003892640e-08 objective function value F of the best point 
         ...
         *  5.6e-05 sigma
         *  2.34 axisratio, ie. quotient between the standard deviations in largest an 
     ...
     * by default, if writeDisplayToFile option is greater than 0
     * see #getPrintLine() 
     */
    ...
    /** 
     * Triangular matrix contening the correlation coefficients. 
     **/
	...
	int i, j;
		...
		rho[i][j] = C[i][j] / Math.sqrt(C[i][i] * C[j][j]);
        }
        return rho;
    }
   ...
   public void printSC() 
   {
    ...
    /** 
     * writes data to files fileNamePrefix fit.dat, ...xmean.dat
...
//                + (System.currentTimeMillis()-timings.start)  + " " + opts.maxTimeFractionForWriteToDefaultFiles);
    ...
    /** 
     * Writes headers (column annotations) to files fit.dat, ...xmean.dat
    ...
    /**
     * Set a flag to repair an ill-conditioned matrix.
     * Further details: http://147.188.192.42/~xin/papers/DongYaoCEC07.pdf
     * 
     * @param repairMatrix
     */
    public void setRepairMatrix(boolean repairMatrix)
    {
    	this.repairMatrix = repairMatrix;
    }
}
